...
static enum adt i2c_get_funcs(int i2cbus) {
    ...
    if (ioctl(file, I2C_FUNCS, &funcs) < 0) ret = adt_unknown;
    else if (funcs & I2C_FUNC_I2C) ret = adt_i2c;
    else if (funcs & (I2C_FUNC_SMBUS_BYTE |
                         I2C_FUNC_SMBUS_BYTE_DATA |
                         I2C_FUNC_SMBUS_WORD_DATA)) ret = adt_smbus;
    else ret = adt_dummy;
...
static int lookup_i2c_bus_by_name(const char *bus_name) {
    ...
    if (i2cbus == -1) fprintf(stderr, "Error: I2C bus name doesn't match any "
                              "bus present!\n");
